  ...
  return (
    <div className="mt-2 text-xs space-y-1">
      <div className="flex gap-2">
        ...
        <span className="text-neutral-800">{data.counterpartyId}</span>
      </div>
      <div className="flex gap-2">
        <span className="text-neutral-500">Times transacted</span>
        <span className="text-neutral-800">{data.timesTransacted}</span>
      </div>
      <div className="flex gap-2">
        <span className="text-neutral-500">Status</span>
        <span className={data.isNewCounterparty ? "text-amber-600" : "text-emerald-600"}>
      ...
      {data.firstSeen && (
        <div className="flex gap-2">
          ...
          <span className="text-neutral-800">{data.firstSeen}</span>
        </div>
      )}
      {data.lastSeen && data.lastSeen !== data.firstSeen && (
        <div className="flex gap-2">
          <span className="text-neutral-500">Last seen</span>
          <span className="text-neutral-800">{data.lastSeen}</span>
        </div>
      )}
    </div>
  );
