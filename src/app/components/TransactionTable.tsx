      ...
      <tbody>
        {transactions.map((tx, i) => (
          <tr key={i} className="border-b border-neutral-100">
            <td className="py-1 pr-4 text-neutral-500">{tx.timestamp}</td>
            <td className="py-1 pr-4 text-neutral-500">{tx.paymentFormat}</td>
            <td className="py-1 pr-4 text-neutral-500 font-mono">{tx.counterparty}</td>
            <td className="py-1 pr-4">
              <span className={tx.direction === "sent" ? "text-red-600" : "text-emerald-600"}>
                {tx.direction === "sent" ? "↑" : "↓"}
              </span>
            </td>
            <td className="py-1 text-right text-neutral-800">{fmt(tx.amountPaid)}</td>
          </tr>
        ))}
      </tbody>
    </table>
